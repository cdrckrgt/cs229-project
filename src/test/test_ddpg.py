...
assert config['action_dimensionality'] == 'continuous', 'You must use a run trained with DDPG with this tester!'
...
for i_episode in range(nb_sims):
    ...
    for t in count():
        ...
        if render: env.render(mode='save', path='../../gifs/{0}.test{1:02d}.frame{2:03d}.png'.format(run, i_episode, t), freq=1, title='{} DDPG solution'.format(run), ep_reward=ep_reward, reward=reward)
        ...
        if done:
            ...
            if render:
                env.render(mode='save', path='../../gifs/{0}.test{1:02d}.frame{2:03d}.png'.format(run, i_episode, t + 1), freq=1, title='{} DDPG solution'.format(run), ep_reward=ep_reward, reward=reward)
